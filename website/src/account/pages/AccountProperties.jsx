...
// Friendly status wording for property owners.
const STATUS_LABEL = {verified: 'Live', pending: 'Under review', rejected: 'Rejected'};
  ...
  return (
    <div>
      <div className="page-header">
        <div>
          <h1>My Properties</h1>
          <p className="page-subtitle">Listings you posted — they go live publicly once our team verifies them.</p>
        </div>
        <Link to="/account/properties/new" className="btn">
            ...
            {loading ? (
              ...
              rows.map(p => (
                <tr key={p.id}>
                  ...
                  <td><span className={badgeClass(p.verificationStatus)}>{STATUS_LABEL[p.verificationStatus] || p.verificationStatus}</span></td>
                  <td>
                    <div style={{display: 'flex', gap: 6, flexWrap: 'wrap'}}>
                      <Link to={`/account/properties/${p.id}/edit`} className="btn ghost sm" style={{padding: '5px 12px', fontSize: 12}}>Edit</Link>
              ...
              <tr>
                <td colSpan={6}>
                  <div style={{textAlign: 'center', padding: '40px 0', color: '#a0a3b1'}}>
                    You haven&apos;t posted any properties yet. <Link to="/account/properties/new" style={{color: '#d98a3e'}}>Post one →</Link>
                  </div>
                </td>
              </tr>
            )}
